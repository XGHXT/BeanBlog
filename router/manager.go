...
func manager(c *fiber.Ctx) error {
	...
	wg.Add(5)
	checkPoolSubmit(&wg, blog.System.Pool.Submit(func() {
		blog.System.DB.Model(model.Article{}).Count(&articleNum)
		wg.Done()
	}))
	checkPoolSubmit(&wg, blog.System.Pool.Submit(func() {
		blog.System.DB.Model(model.Comment{}).Count(&commentNum)
		wg.Done()
	}))
	checkPoolSubmit(&wg, blog.System.Pool.Submit(func() {
		blog.System.DB.Raw(`select count(*) from (select tags,count(tags) from (select unnest(tags) as tags from articles) t group by tags) ts;`).Scan(&tn)
		wg.Done()
	}))
	checkPoolSubmit(&wg, blog.System.Pool.Submit(func() {
		blog.System.DB.Select("created_at").Order("created_at DESC").Take(&lastArticle)
		wg.Done()
	}))
	checkPoolSubmit(&wg, blog.System.Pool.Submit(func() {
		blog.System.DB.Select("created_at").Order("created_at DESC").Take(&lastComment)
		wg.Done()
	}))
	wg.Wait()
